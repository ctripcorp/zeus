...
selfInfoApp.controller('selfInfoController', function ($scope, $http, $q, $window) {
    ...
    $scope.changeAppIdSelection = function () {
        ...
        $http(request).success(function (response) {
            var data = _.map(response['groups'], function (item) {
                ...
                var app = appId + '(' + $scope.apps[appId]['chinese-name'] + ')';
                var id = item.id;
                var name = item.name;
                var javailable = true;
                ...
                if (!lavailable) {
                    errorMsg = '当前Group是Java应用，请到 <a href="/portal/statistics/statistics-netjava#?env=' + G.env + '">.NET转Java页面</a>申请流量灰度';
                } else {
                    if (!(ravailable && savailable && pavailable && vsavailable)) {
                        if (!ravailable) errorMsg = '当前Group找不到相关联的应用，请联系 <a href="mailto:OPS_APP@Ctrip.com?subject=SLB上新建应用迁移策略失败&body=我正在通过SLB新建应用迁移策略。新应用的AppId =' + appId + ', 系统提示尚未关联我的老应用。:%E2%80%A8%E2%80%A8%E2%80%A8%E2%80%A8">OPS Team</a> 添加关联';
                        else if (!savailable) errorMsg = '当前Group尚未激活，请联系<a href="mailto:OPS_APP@Ctrip.com?subject=SLB新建应用迁移策略&body=我正在通过SLB新建应用迁移策略,新应用AppId=' + appId + ',系统提示，当前应用尚未激活%E2%80%A8%E2%80%A8%E2%80%A8%E2%80%A8">OPS Team</a>激活后再创建应用迁移策略测试应用';
                        else if (!pavailable) errorMsg = '当前Group已经存在 <a href="/portal/policy#?env=' + $scope.env + '&policyId=' + hasPolicy.id + '">{{resource["navs"]["navs_app_policy"]}}</a>，不能再创建了!';
                        else if (!vsavailable) errorMsg = '当前Group存在多个访问入口，但是多个访问入口之间的Path不相同,请联系<a href="mailto:OPS_APP@Ctrip.com?subject=SLB上新建应用迁移&body=我正在通过SLB新建应用迁移策略。 新应用的AppId=' + appId + '系统提示:存在多个Virtual Servers，但是多个Virtual Server之间的Path不相同 %E2%80%A8%E2%80%A8%E2%80%A8%E2%80%A8">OPS Team</a> 统一Path';
                    }
                }
    ...
    $scope.changeAppIdDeactivateSelection = function () {
        ...
        var rightUrl = G[$scope.env].urls.api + '/api/auth/apply/mail?userName=' + $scope.query.login + '&op=ACTIVATE,DEACTIVATE,UPDATE,READ,DELETE&type=Policy';
        ...
        $q.all(
            userPolicyApp.getPolicyByAppId(request, appId, $scope.apps, $scope.groups, rightUrl, function (result) {
                ...
                if (result && result.length > 0) {
                    ...
                    $scope.appidNotHasPolicyError = '当前应用没有关联任何灰度策略!';
                }
            })
        );
    ...
    $scope.confirmDeactivatePolicy = function () {
        ...
        var deactivatePolicyRequest = {
            url: G.baseUrl + '/api/deactivate/policy?policyId=' + policy.id+"&description=remove",
            method: 'GET'
        };
        var deletePolicyRequest = {
            url: G.baseUrl + '/api/policy/delete?policyId=' + policy.id+"&description=remove",
            method: 'GET'
        };
    ...
    $scope.firstToSecond = function (event) {
        ...
        $http(request).success(function (response, code) {
            ...
            if (cgroups.length == 0) {
                $scope.firstToSecondErrorMessage = 'A应用所关联的B应用AppId: ' + firstGroupTargetedApp + '. 当前找不到该关联的应用。　请联系 SLB Team获取帮助';
            ...
            var data = _.map(cgroups, function (item) {
                ...
                var secondVses = _.map(item['group-virtual-servers'], function (t) {
                    return {vs: t['virtual-server'].id, path: t.path.toLowerCase()}
                });
                ...
                if (!(firstGroupRight && rightavailable && statusavailable && policyavailable)) {
                    if (!(rightavailable && firstGroupRight)) {
                        isAccessLink = true;
                        errorMsg = '您没有操作当前应用的权限，<a class="fast-apply">点此快速申请权限</a>';
                    }
                    else if (!statusavailable) errorMsg = '当前Group尚未激活，请联系<a href="mailto:OPS_APP@Ctrip.com?subject=SLB上迁移应用&body=请添加咨询问题列表:%E2%80%A8%E2%80%A8%E2%80%A8%E2%80%A8">OPS Team</a>激活后再创建应用迁移策略';
                    else if (!policyavailable) errorMsg = '当前Group已经创建过 <a href="/portal/policy#?env=' + $scope.env + '&policyId=' + hasPolicy.id + '">应用迁移策略</a>，不能再创建了!';
                    else if (!vsavailable) errorMsg = '当前Group对应的Virtual Server信息与Java Group不一致，<a href="mailto:OPS_APP@Ctrip.com?subject=SLB上应用迁移策略&body=请添加咨询问题列表:%E2%80%A8%E2%80%A8%E2%80%A8%E2%80%A8">OPS Team</a> 统一Path';
                }
                return {
                    appId: appId,
                    app: app,
    ...
    $scope.secondToThird = function (event) {
        ...
        secondGroup.type = 'B';
        secondGroup.desc = secondGroup['app-id'] + '(老)';
        ...
        firstGroup.type = 'A';
        firstGroup.desc = firstGroup['app-id'] + '(新)';
    ...
    $scope.getDescByType = function (type) {
        switch (type) {
            case 'A':
                return '(新应用默认权重0%，不接受任何流量，创建完成后可调整)';
                break;
            case 'B':
                return '(老应用默认权重100%，暂时接收所有流量)';
                break;
            default :
                break;
        }
    ...
    $scope.thirdToForth = function (event) {
        ...
        var request = {
            method: 'POST',
            url: G.baseUrl + '/api/policy/new?description=update',
            data: policy
        };
        var loading = "<img src='/static/img/spinner.gif' /> 正在创建";
                ...
                swal({
                    ...
                    if (isConfirm) {
                        $scope.forceDo(request, function () {
                            ...
                            swal({
                                title: "强制操作仍然失败!",
                                text: "强制操作失败。错误信息：" + message + ',请联系SLB Team寻求帮助！',
    ...
    $scope.activatePolicy = function (pid) {
        ...
        $scope.getPolicyById(id, function () {
            ...
            var ptext = '线下版本' + ($scope.tobeActivatedPolicyData.version ? $scope.tobeActivatedPolicyData.version : '-');
            var ctext = "线上版本" + ($scope.onlinePolicyData.version ? $scope.onlinePolicyData.version : '-');
    ...
    $scope.confirmDeactivateNetApp = function () {
        ...
        var loading = "<img src='/static/img/spinner.gif' /> 正在下线.NET应用";
        $('#operationConfrimModel').find(".modal-body").html(loading);
        var success = '<i class="status-green fa fa-check" /> 下线成功,将于2s后自动关闭';
    ...
    $scope.confirmActivatePolicy = function () {
        ...
        var success = '<i class="status-green fa fa-check" /> 激活成功,将于2s后自动关闭';
        $('#operationConfrimModel').find(".modal-body").html(loading);
        ...
        $scope.do(request,
            ...
            function (message) {
                ...
                swal({
                    title: "激活失败，是否强制激活?",
                    text: "激活失败，错误信息：" + message,
                    type: "warning",
                    showCancelButton: true,
                    confirmButtonColor: "#DD6B55",
                    confirmButtonText: "强制激活!",
                    closeOnConfirm: true
                }, function (isConfirm) {
                    if (isConfirm) {
                        $scope.forceDo(request, function () {
    ...
    $scope.changePolicyWeight = function (id) {
        $scope.policyId = id;
        $('.origin-footer').show();
        var c = _.extend($scope.ownedTraffics, $scope.focusedTraffics);
        var p = c[id];
        if (!p) alert('Could not find the traffic policy');
            ...
            if (!left) {
                alert('当前Policy中不存在 relatedappid映射，请联系 OPS Team获取帮助！');
                return;
            }
    ...
    $scope.saveWeight = function (event) {
        ...
        var request = {
            method: 'POST',
            url: G.baseUrl + '/api/policy/update?description=update',
            data: p
        };
        var loading = "<img src='/static/img/spinner.gif' /> 正在更新";
        $('#operationConfrimModel').find(".modal-body").html(loading);
        $('#operationConfrimModel').find(".modal-title").html("正在更新权重");
    ...
    $scope.loadData = function (hashData, owner) {
        ...
        var result = {};
        var trafficsOwnerString = G.baseUrl + '/api/policies?type=extended&anyProp=target:应用迁移';
        ...
        var policies = userPolicyApp.getPolicyCollection('', trafficsOwnerString, existingString, result);
        if (hashData.keyword) {
        ...
        $q.all(
            ...
            if ((!type || type.toLowerCase() == 'new') && hashData.appId && !$scope.loaded) {
                ...
                if ($scope.apps[targetJavaAppId]) {
                    ...
                    $('#errorConfrimModel').modal('show').find('.modal-body').html(targetJavaAppId + ', 在SLB 上找不到，可能不存在或者尚未接入SLB. 请联系 <a href="mailto:slb@test.com">SLB Team</a>需求帮助');
                }
                $scope.loaded = true;
            }
            ...
            var summary = userPolicyApp.getSummaryData(c);
    ...
    $scope.hashChanged = function (hashData) { $scope.resource = H.resource;
       ...
       $scope.env = 'pro';
        ...
        $http(request).success(function (response, code) {
            if (code != 200) {
                exceptionNotify('失败', '失败原因: ' + response.message, null);
                return;
            } else {
                $scope.loginUserInfo.user = response.name;
                $scope.query.userId = response.name;
    ...
    $scope.navigateTo = function (item) {
        ...
        switch (item) {
            case 'activated': {
                url = '/portal/policies#?env=' + G.env + '&policyStatus=已激活&policyTarget=应用迁移';
                break;
            }
            case 'tobeactivated': {
                url = '/portal/policies#?env=' + G.env + '&policyStatus=有变更&policyTarget=应用迁移';
                break;
            }
                ;
            case 'deactivated': {
                url = '/portal/policies#?env=' + G.env + '&policyStatus=未激活&policyTarget=应用迁移';
                break;
            }
            case 'domain':
                break;
