...
usersInfoApp.controller('usersInfoController', function ($scope, $http, $q) {
    ...
    $scope.selectbuName = function (o) {
        if (o) {
            $scope.updateUser.bu = o.originalObject.id;
        }
    };
    ...
    $('#updateUserBt').click(function (e) {
        ...
        if (review) {
            ...
            var getRequest = {
                url: G.baseUrl + '/api/auth/user?userName=' + userName,
                method: 'GET'
            };
            $http(getRequest).success(function (response, code) {
                if (code == 200) {
                    response['bu'] = bu;
                    var request = {
                        method: 'POST',
                    ...
                    alert('Failed to get user information, error code:' + code);
                }
            });
        }
    });
