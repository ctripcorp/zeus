...
operationLogApp.controller('operationLogController', function ($scope, $http, $q) {
    ...
    $scope.showNewRuleBt = function () {
        var offline = $scope.group.offline;
        if (!offline) return false;
        return A.canDo("Group", "UPDATE", offline.id);
    };
    ...
    $scope.showDeleteComposite = function () {
        ...
        if (typeCount) {
            ...
            return _.values(compiste) > 1 || eachCompisteCount.length > 1;
        }
    ...
    $scope.saveCondition = function () {
        ...
        var rule = {
            'target-type': 'Group',
            'target-id': groupId,
            'rule-type': 'REQUEST_INTERCEPT_FOR_ANTIBOT_RULE',
            'name': 'REQUEST_INTERCEPT_FOR_ANTIBOT_RULE_' + groupId,
    ...
    function getConditionText(target, func, value) {
        ...
        } else if (target.indexOf('$merge') == 0) {
            str += '<div>当复合条件:<span>' + target.substring(7, target.length - 1) + '</span> <b class="function">' + func + '</b> ' + value + '</div>';
        }
        str += '</div>';
        return str;
    }
    ...
    function getTarget(target) {
        if (target.toLowerCase().indexOf('$header_') == 0) {
            ...
            return 'cip';
        } else if (target.toLowerCase().indexOf('$merge') == 0) {
            return 'merge';
        }
        return target;
    }
    ...
    function getViewData(obj, oldTarget, newTarget) {
        if (newTarget == 'header' || newTarget == 'cookie' || newTarget == 'arg') {
            ...
            obj.ip = obj.value;
        } else if (newTarget == 'merge') {
            obj.key = oldTarget.substring(7, oldTarget.length - 1);
        }
    ...
    $scope.loadData = function () {
        ...
        $q.all([onlineRequest, offlineRequest]).then(function () {
            var offlineGroup = $scope.group['offline'];
            pagerules = _.filter(offlineGroup['rule-set'], function (v) {
                return v['rule-type'] == 'REQUEST_INTERCEPT_FOR_ANTIBOT_RULE';
            });
