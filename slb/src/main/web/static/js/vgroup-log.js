...
summaryInfoApp.controller('summaryController', function ($scope, $http, $q) {
    ...
    $scope.getAllTargets = function () {
        $http.get(G.baseUrl + "/api/vgroup?groupId=" + $scope.query.groupId + "&type=info").success(
            function (res) {
                $scope.target = {};
...
var opsLogQueryApp = angular.module('opsLogQueryApp', ['ui.bootstrap', 'ui.bootstrap.datetimepicker', 'angucomplete-alt', 'http-auth-interceptor']);
opsLogQueryApp.controller('opsLogQueryController', function ($scope, $http, $filter) {
    ...
    $scope.loadData = function (hashData) {
        ...
        if (groupId) {
            groupsRequest.url = G.baseUrl + '/api/vgroup';
    ...
    $scope.getGroupRef = function () {
        var group = $scope.data.group;
        if (group)
            return '/portal/vgroup#?env=' + $scope.env + '&groupId=' + group['id'];
    };
            ...
            columns: [[
...

                {
                    ...
                    formatter: function (value, row, index) {
                        if (value) {
                            ...
                            return '<span class="' + streeclass + '" style="word-break: break-all">' + value + '</span>';
                        }
                        else return '-';
                    }
                },
                ...
                },
                {
                    ...
                    formatter: function (value, row, index) {
                        ...
                        var query = 'group_id:' + groupId + ' AND group_server:' + row['target-ip'] + '*';
