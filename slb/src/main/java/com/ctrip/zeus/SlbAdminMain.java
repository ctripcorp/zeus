...
public class SlbAdminMain {
    ...
    private static void setSystemProperties() {
        ...
        S.setPropertyDefaultValue("server.port", "80");
        S.setPropertyDefaultValue("archaius.deployment.applicationId", "slb-admin");
        S.setPropertyDefaultValue("archaius.deployment.environment", "regression");
        S.setPropertyDefaultValue("server.www.base-dir", new File("").getAbsolutePath() + "/slb/src/main/web");
        S.setPropertyDefaultValue("server.temp-dir", new File("").getAbsolutePath() + "/target/temp");
        S.setPropertyDefaultValue("CONF_DIR", new File("").getAbsolutePath() + "/conf/" + System.getProperty("archaius.deployment.environment"));
        S.setPropertyDefaultValue("appId", "100000716");

        try {
            Properties properties = new Properties();
            properties.load(SlbAdminMain.class.getResourceAsStream("/zeus-git.properties"));
