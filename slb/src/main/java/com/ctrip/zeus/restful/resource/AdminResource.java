...
public class AdminResource {
    ...
    public Response listConfInfos(@Context final HttpHeaders hh,
        ...
        for (Slb slb : slbRepository.list(executer.run())) {
            authService.authValidate(UserUtils.getUserName(request), ResourceOperationType.ADMIN_INFO, ResourceDataType.Slb, slb.getId());
    ...
    public Response getConfInfo(@Context final HttpHeaders hh,
        ...
        if (slbId == null || slbId.longValue() == 0L)
            throw new ValidationException("Slb id cannot be found. slbId:" + slbId);

        Slb slb = slbRepository.getById(slbId);
        if (slb == null)
            throw new ValidationException("Slb cannot be found. slbId:" + slbId);
        authService.authValidate(UserUtils.getUserName(request), ResourceOperationType.ADMIN_INFO, ResourceDataType.Slb, slb.getId());
