...
public class PathParseHandler {
    ...
    private static final char[] INVALID_CHARS_BEFORE_STANDARD_SUFFIX = {'/'};
    ...
    private int enumeratePathValues(char[] pathArray, List<String> prefix, int start, int depth, char startSymbol) throws GrammarException {
        ...
        for (; i < pathArray.length; i++) {
            ...
            switch (pathArray[i]) {
                case '(':
                    i = startBrackets(pathBuilder, prefix, i, pathArray, depth);
                    break;
                case ')':
                    endBrackets(pathBuilder, prefix, startSymbol, pathArray, i);
                    return i;
                case '|':
    ...
    private void endBrackets(StringBuilder pathBuilder, List<String> prefix, char startSymbol, char[] pathArray, int i) throws GrammarException {
        if (pathBuilder.length() > 0) {
            String v = pathBuilder.toString();
            pathBuilder.setLength(0);
            if (prefix.size() == 0) {
    ...
    private int startBrackets(StringBuilder pathBuilder, List<String> prefix, int i, char[] pathArray, int depth) throws GrammarException {
        List<String> subRoot;
        if (Arrays.equals(STANDARD_SUFFIX_PATTERN, Arrays.copyOfRange(pathArray, i, i + 8))) {
            if (i != 0) {
                char charBeforeSuffix = pathArray[i - 1];
                if (ArrayUtils.contains(INVALID_CHARS_BEFORE_STANDARD_SUFFIX, charBeforeSuffix)) {
                    throw new GrammarException("Invalid character '" + charBeforeSuffix + "' is found before the standard path suffix "
                            + new String(STANDARD_SUFFIX_PATTERN) + ".");
                }
            }
