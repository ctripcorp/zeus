...
public abstract class AbstractMultiRelMaintainer<T, W, X> implements MultiRelMaintainer<W, X> {
    ...
    protected AbstractMultiRelMaintainer(Class<T> domainClazz, Class<X> viewClazz) {
        ...
        mGetId = MethodCache.get(clazzName + "#getId");
        mGetVersion = MethodCache.get(clazzName + "#getVersion");
        if (mGetId == null) {
            try {
                mGetId = viewClazz.getMethod("getId");
                MethodCache.put(clazzName + "#getId", mGetId);
        ...
        if (mGetVersion == null) {
            try {
                mGetVersion = viewClazz.getMethod("getVersion");
                MethodCache.put(clazzName + "#getVersion", mGetVersion);
            } catch (NoSuchMethodException e) {
    ...
    public void refreshOffline(X object) throws Exception {
        ...
        versions[OFFSET_OFFLINE] = getObjectVersion(object);
    ...
    protected Long getObjectId(X object) {
        if (mGetId != null) {
            try {
                return (Long) mGetId.invoke(object);
            } catch (Exception e) {
    ...
    protected Integer getObjectVersion(X object) {
        if (mGetVersion != null) {
            try {
                return (Integer) mGetVersion.invoke(object);
            } catch (Exception e) {
    ...
    protected abstract void setDo(X object, W value, T target) throws Exception;
