...
public class VersionUtils {
    ...
    public static int[] getVersionByMode(SelectionMode mode, int offline, int online) {
        return getVersionByMode(mode, offline, online, null);
    }

    public static int[] getVersionByMode(SelectionMode mode, int offline, int online, Integer canary) {
        switch (mode) {
            ...
            case CANARY: {
                return canary == null ? new int[0] : new int[]{canary};
            }
            case OFFLINE_FIRST:
            default: {
