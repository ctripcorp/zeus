...
public class DefaultVirtualServerValidator implements VirtualServerValidator {
    ...
    private DynamicStringProperty portWhiteList = DynamicPropertyFactory.getInstance().getStringProperty("port.whitelist", "80,443,8080", new Runnable() {
        @Override
        public void run() {
    ...
    public void setGroupCriteriaQuery(GroupCriteriaQuery groupCriteriaQuery) {
        this.groupCriteriaQuery = groupCriteriaQuery;
    }
    ...
    @Override
    public void validateCanBeDeactive(VirtualServer vs) throws ValidationException {
        if (vs == null){
            return;
        }
        List<Rule> rules = vs.getRuleSet();
        if (rules == null || rules.size() == 0) {
            return;
        }
        for (Rule rule : rules) {
            if (rule.getRuleType().equalsIgnoreCase(RuleType.HTTP_REDIRECT_RULE.getName())) {
                throw new ValidationException("Vs Have HTTP_REDIRECT_RULE. Should Not Deactivate.");
            }
        }
    }
    ...
    public void removable(Long vsId) throws Exception {
        SlbVsStatusR check = slbVsStatusRMapper.selectOneByExampleSelective(new SlbVsStatusRExample().createCriteria().andVsIdEqualTo(vsId).example());
    ...
    public void checkRestrictionForUpdate(VirtualServer target) throws Exception {
        SlbVsStatusR check = slbVsStatusRMapper.selectOneByExampleSelective(new SlbVsStatusRExample().createCriteria().andVsIdEqualTo(target.getId()).example());
        if (check == null) {
