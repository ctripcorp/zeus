    ...
    public IdVersion[] run(CriteriaQueryFactory criteriaQueryFactory) throws Exception {
        ...
        if (resource.equals("group")) {
            ...
            if (alias == null) {
                ...
                // TODO: Switch to a better way of querying all the groups.
                String groupTypeParamValue = getParamValue("groupType");
                if (tmp.size() > 0 && !"all".equalsIgnoreCase(groupTypeParamValue)) {
                    // Something is found and user doesn't choose to keep all types of groups.
