...
import com.ctrip.zeus.LogFormat;
...
public class AccessLogStatsAnalyzer implements LogStatsAnalyzer {
    ...
    private static final String AccessLogSeparator = LogFormat.getSeparator();
    ...
    public AccessLogStatsAnalyzer() {
        this(new LogStatsAnalyzerConfigBuilder()
                ...
                .addLogFormat(new AccessLogStateMachineFormat(AccessLogFormat, AccessLogSeparator).generate())
    ...
    public AccessLogStatsAnalyzer(LogStatsAnalyzerConfig config) {
        ...
        logParser = new AccessLogStateMachineParser(config.getDefaultLineFormat(),config.getLineFormats());
    ...
    public static class LogStatsAnalyzerConfigBuilder {
        private List<LineFormat> logFormats = new ArrayList<>();
        ...
        public LogStatsAnalyzerConfigBuilder setDefaultLogFormat(LineFormat logFormat) {
            this.logFormat = logFormat;
            return this;
        }

        public LogStatsAnalyzerConfigBuilder addLogFormat(LineFormat logFormat) {
            this.logFormats.add(logFormat);
            return this;
        }
        ...
        public LogStatsAnalyzerConfig build() {
            ...
            LogStatsAnalyzerConfig config = new LogStatsAnalyzerConfig(delegatorRegistry)
                    .setNumberOfConsumers(numberOfConsumers)
                    .setLogTracker(new AccessLogTracker(strategy));
            config.getLineFormats().addAll(logFormats);
            config.setDefaultLineFormat(logFormat);
            return config;
        }
    }
}
