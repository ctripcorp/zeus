...
import com.ctrip.zeus.logstats.analyzer.nginx.filter.ErrorLogHeaderSizeFilter;
import com.ctrip.zeus.logstats.common.LineFormat;
import com.ctrip.zeus.logstats.parser.ErrorLogStateMachineParser;
 ...
 */
public class ErrorLogStatsAnalyzer implements LogStatsAnalyzer {
    ...
    public ErrorLogStatsAnalyzer() {
        this(new LogStatsAnalyzerConfigBuilder()
                .isStartFromHead(false)
                .setLogFilename("/opt/logs/nginx/error.log")
                .setReadBufferSize(1024 * 5)
                .build());
    }

    public ErrorLogStatsAnalyzer(LogStatsAnalyzerConfig config) {
        this.config = config;
        logTracker = config.getLogTracker();
        ErrorLogStateMachineParser parser = new ErrorLogStateMachineParser();
        parser.addFilter(new ErrorLogHeaderSizeFilter());
